...
# Path to shell5 base image
shell5_path = r'C:\Users\owner\.gemini\antigravity\brain\96f9f0cb-21ba-4acc-94ca-5ea9095d0775\ninnik_3branches_shell5_1784750842605.jpg'
out_png = os.path.join(cropped_dir, 'ninnik_3sculptures_emoji.png')

if os.path.exists(shell5_path):
    img = Image.open(shell5_path).convert('RGB')
    arr = np.array(img)
    h, w, c = arr.shape
    
    # 1. Clean up paint dripping on downward branch tip if any
    # Paint tip is located near the bottom left/center branch tip
    # Apply vivid orange #FF5500 color neatly to the very tip of the branch without drips
    bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
    
    # Extract outer background cleanly
    ...
    print("Successfully processed shell5 transparent PNG:", out_png, "Size:", result.size)
